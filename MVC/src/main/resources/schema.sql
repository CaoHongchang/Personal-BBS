...
ALTER TABLE `ssm`.`topic`
    ADD COLUMN `createdTime` BIGINT(20) NOT NULL AFTER `userId`,
    ADD COLUMN `updatedTime` BIGINT(20) NOT NULL AFTER `createdTime`;
...
#   修改表语句:Topic
ALTER TABLE `ssm`.`topic`
    ADD COLUMN `boardId` INT(11) NOT NULL AFTER `updatedTime`;
